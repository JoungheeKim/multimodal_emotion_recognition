...
@dataclass
class AudioLanguageShallowMultiConfig(AudioLanguageModelConfig):
    name:str = 'shallow_fusion_multi'
...
class AudioLanguageShallowMultiModel(AudioLanguageModel):
    def __init__(self, model_cfg:AudioLanguageShallowMultiConfig, ad_cfg:AudioFeatureConfig, lm_cfg:LanguageFeatureConfig, label_cfg:LabelPreprocessConfig):
        super(AudioLanguageShallowMultiModel, self).__init__(model_cfg, ad_cfg, lm_cfg, label_cfg)
    ...
    def get_loss(self, net_output, labels):
        ...
        if self.multitune_start_step <= self.step and self.multitune_end_step >= self.step:
        ...
        if self.audiotune_start_step <= self.step and self.audiotune_end_step >= self.step:
        ...
        if self.texttune_start_step <= self.step and self.texttune_end_step >= self.step:
