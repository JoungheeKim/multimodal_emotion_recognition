...
@dataclass
class AudioLanguageShallowPointerConfig(AudioLanguageModelConfig):
    name:str = 'shallow_fusion_pointer'
...
class AudioLanguageShallowPointerModel(AudioLanguageModel):
    def __init__(self, model_cfg:AudioLanguageShallowPointerConfig, ad_cfg:AudioFeatureConfig, lm_cfg:LanguageFeatureConfig, label_cfg:LabelPreprocessConfig):
        super(AudioLanguageShallowPointerModel, self).__init__(model_cfg, ad_cfg, lm_cfg, label_cfg)
        ...
        self.am_portion = nn.Linear(am_concat_dim, 1)
        ...
        self.lm_portion = nn.Linear(lm_concat_dim, 1)
        ...
        self.multi_portion = nn.Linear(am_concat_dim + lm_concat_dim, 1)
    ...
    def process(self, input_ids, audio, token_type_ids=None, attention_mask=None, padding_mask=None, **kwargs):
        ...
        output_portion = self.sigmoid(self.multi_portion(self.relu(output)))

        output = (self.proj(output) * output_portion) + (self.am_proj(audio_encoding) * audio_portion) + (self.lm_proj(text_encoding) * text_portion)
