...
@dataclass
class AudioLanguageDeepPointerConfig(AudioLanguageModelConfig):
...
class AudioLanguageDeepPointerModel(AudioLanguageModel):
    def __init__(self, model_cfg:AudioLanguageDeepPointerConfig, ad_cfg:AudioFeatureConfig, lm_cfg:LanguageFeatureConfig, label_cfg:LabelPreprocessConfig):
        super(AudioLanguageDeepPointerModel, self).__init__(model_cfg, ad_cfg, lm_cfg, label_cfg)
    ...
    def process(self, input_ids, audio, token_type_ids=None, attention_mask=None, padding_mask=None, **kwargs):
        ...
        audio_portion = self.sigmoid(self.am_portion(audio_encoding))
        ...
        text_portion = self.sigmoid(self.lm_portion(text_encoding))
        ...
        fusion_portion = self.sigmoid(self.fusion_portion(features))

        return (output * fusion_portion) + (only_audio_encoding * audio_portion) + (only_text_encoding * text_portion)
