...
@dataclass
class AudioLanguageTransformerPointerConfig(AudioLanguageModelConfig):
    name:str = 'multimodal_transformer_pointer'
...
class AudioLanguageTransformerPointerModel(AudioLanguageModel):
    def __init__(self, model_cfg:AudioLanguageTransformerPointerConfig, ad_cfg:AudioFeatureConfig, lm_cfg:LanguageFeatureConfig, label_cfg:LabelPreprocessConfig):
        super(AudioLanguageTransformerPointerModel, self).__init__(model_cfg, ad_cfg, lm_cfg, label_cfg)
        ...
        self.am_portion = nn.Linear(am_hidden_dim, 1)
        ...
        self.lm_portion = nn.Linear(lm_hidden_dim, 1)
        ...
        self.multi_portion = nn.Linear(model_cfg.hidden_dim, 1)
    ...
    def process(self, input_ids, audio, token_type_ids=None, attention_mask=None, padding_mask=None, **kwargs):
        ...
        if not self.am_last_hidden:
            only_audio_encoding = self._masked_mean(audio_encoding, audio_pad_false_mask, dim=1)
        else:
            only_audio_encoding = audio_encoding[:,0,:]
        only_audio_encoding = self.final_dropout(only_audio_encoding)
        audio_portion = self.sigmoid(self.am_portion(only_audio_encoding))
        ...
        if not self.lm_last_hidden:
            only_text_encoding = self._masked_mean(text_encoding, text_pad_false_mask, dim=1)
        else:
            only_text_encoding = text_encoding[:,0,:]
        only_text_encoding = self.final_dropout(only_text_encoding)
        text_portion = self.sigmoid(self.lm_portion(only_text_encoding))
        ...
        output_portion = self.sigmoid(self.multi_portion(output))

        output = self.relu(self.fc2(output))
        output = features + output
        
        return (self.proj(output) * output_portion) + (self.am_proj(only_audio_encoding) * audio_portion) + (self.lm_proj(only_text_encoding) * text_portion)
