...
@dataclass
class AudioLanguageDeepPointerV2Config(AudioLanguageModelConfig):
    name:str = 'deep_fusion_pointer'
...
class AudioLanguageDeepPointerV2Model(AudioLanguageModel):
    def __init__(self, model_cfg:AudioLanguageDeepPointerV2Config, ad_cfg:AudioFeatureConfig, lm_cfg:LanguageFeatureConfig, label_cfg:LabelPreprocessConfig):
        super(AudioLanguageDeepPointerV2Model, self).__init__(model_cfg, ad_cfg, lm_cfg, label_cfg)
        ...
        self.relu = nn.ReLU()
        self.fc = nn.Linear(model_cfg.hidden_dim, model_cfg.hidden_dim)
        self.proj = nn.Linear(model_cfg.hidden_dim, self.num_class)
        self.fusion_portion = nn.Linear(model_cfg.hidden_dim, 1)
    ...
    def process(self, input_ids, audio, token_type_ids=None, attention_mask=None, padding_mask=None, **kwargs):
        ...
        audio_portion = self.sigmoid(self.am_portion(self.relu(audio_encoding)))
        only_audio_encoding = self.am_proj(audio_encoding)
        ...
        text_portion = self.sigmoid(self.lm_portion(self.relu(text_encoding)))
        only_text_encoding = self.lm_proj(text_encoding)
        ...
        output = self.relu(self.fc(features))
        output = self.final_dropout(output)
        output = output + feature

        fusion_portion = self.sigmoid(self.fusion_portion(output))
        fusion_output = self.proj(output)
        

        return (fusion_output * fusion_portion) + (only_audio_encoding * audio_portion) + (only_text_encoding * text_portion)
