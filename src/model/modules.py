...
class CrossmodalTransformer(nn.Module):
    ...
    def forward(self, x_query, x_key=None, key_pad_mask=None, slf_attn_mask=None):
        ...
        for layer in self.layers:
            x_query = layer(x_query, x_key, key_pad_mask, slf_attn_mask, causal_attn_mask=self.causal_attn_mask)
        return x_query.transpose(0, 1)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x_query, x_key=None,
                key_pad_mask=None, slf_attn_mask=None, causal_attn_mask=False):
        """
        x : input of the encoder layer -> (L, B, d)
        """
        if x_key is not None:
            x = self.transformer(
                x_query, x_key, x_key, key_pad_mask=key_pad_mask, causal_attn_mask=causal_attn_mask
            )

        else:
            x = self.transformer(
                x_query, x_query, x_query, key_pad_mask=key_pad_mask, causal_attn_mask=causal_attn_mask,
            )
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, query, key, value, key_pad_mask=None, causal_attn_mask=False):
        mask = self.get_future_mask(query, key) if causal_attn_mask else None
        # Do layernorm befor self-attention
        query, key, value = [self.layernorm(x) for x in (query, key, value)]
        x = self.self_attn(query, key, value, key_padding_mask=key_pad_mask, attn_mask=mask)[0]
